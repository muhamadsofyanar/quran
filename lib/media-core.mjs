...
function validCorpusEntries(ayahs) {
...
export function alignTranscriptSequence(
  parts,
  ayahs,
  options = {}
) {
  ...
  const firstMatchThreshold =
    Math.max(
      0.3,
      Math.min(
        0.95,
        Number(
          options.firstMatchThreshold ??
            0.3
        )
      )
    );
...
function segmentText(
  ...
  if (mode === "tafsir") return tafsir;
  if (mode === "all") return [arabic, translation, tafsir].filter(Boolean).join("\n");
...
Style: Quran,Traditional Arabic,${portrait ? 58 : 64},&H00FFFFFF,&H000000FF,&H00102018,&H66000000,-1,0,0,0,100,100,0,0,1,3,1,2,80,80,110,1
