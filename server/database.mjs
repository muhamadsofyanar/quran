// @phase TQ-03 — PostgreSQL connection, migration, and transaction boundary.
...
export async function migrateDatabase() {
  if (!databaseConfigured()) return { configured: false, migrated: false };
  const sql = await readFile(new URL("./migrations/001-production.sql", import.meta.url), "utf8");
...
export async function databaseStatus() {
  ...
  try {
    ...
    return { configured: true, healthy: true, database: result.rows[0].name };
  } catch (error) {
